...
async function run() {
  ...
  const query = `
    ...
    SET "desc" = $1, ingredients = $2 
    WHERE id = $3
  `;
  await client.query(query, [
    'Домашний торт по классическому рецепту. Сметанные коржи прослоены нежным сметанным кремом.',
    'Мука, сахар, яйцо, сметана, сливочное масло, разрыхлитель. Аллергены: глютен, молоко, яйца.',
    'cake_mishka_na_severe'
  ]);
  
  console.log("Updated cake_mishka_na_severe in DB.");
  await client.end();
}
