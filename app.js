...
function completeKaspiOrder() {
  ...
  if (p) {
    ...
    const qtyVal = parseInt(qtyValEl ? qtyValEl.textContent : "1") || 1;
...
function openKaspiQrModal() {
  ...
  if (p) {
    ...
    if (kaspiProdName) kaspiProdName.textContent = displayName;
    if (kaspiProdQty) kaspiProdQty.textContent = `x${qtyVal}`;
    if (kaspiTotalPrice) kaspiTotalPrice.textContent = `${total.toLocaleString()} ₸`;
    if (kaspiQrAmount) kaspiQrAmount.textContent = `${total.toLocaleString()} ₸`;
...
function setupKaspiQrCheckout() {
  ...
  quickKaspiBtn.addEventListener("click", openKaspiQrModal);

  btnKaspiGenerate.addEventListener("click", generateKaspiQr);

  btnKaspiComplete.addEventListener("click", completeKaspiOrder);
}
